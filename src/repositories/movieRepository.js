...
export async function createMovie(payloadData) {
    ...
    const query = `
          ...
          VALUES (?, ?, ?, ?, ?, ?, ?)`;
    const value = [title, genreJSON, release_year, rating, deskripsi, image, update_status];
...
export const createNewMovie = async(payload) =>{
    const { title, genreJSON, release_year, rating, deskripsi, image, update_status } = payload;
    const query = `
          INSERT INTO movies 
          (title, genre, release_year, rating, deskripsi, image, update_status)
          VALUES (?, ?, ?, ?, ?, ?, ?)`;
